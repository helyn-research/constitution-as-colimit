...
\begin{proof}
$F_\Sigma$ is an accessible endofunctor on $\mathbf{Set}$ (it is a composite of accessible functors: product with a constant and exponentiation by a constant). $F_\Sigma$ preserves weak pullbacks. $\mathbf{Set}$ is locally presentable. By \cite{adamek1994}, $\text{Coalg}(F_\Sigma)$ is locally presentable and in particular cocomplete. The forgetful functor creates the colimits.
\end{proof}
