...
Let $\Sigma$ be a set (the signal space). The \emph{Moore machine functor} $F_\Sigma : \mathbf{Set} \to \mathbf{Set}$ is defined by:
...
\label{thm:closure}
$\text{Coalg}(F_\Sigma)$ has all colimits. The forgetful functor $U : \text{Coalg}(F_\Sigma) \to \mathbf{Set}$ strictly creates them.
\end{theorem}

\begin{proof}
$F_\Sigma$ is an accessible endofunctor on $\mathbf{Set}$ (it is a composite of accessible functors: product with a constant and exponentiation by a constant). $F_\Sigma$ preserves weak pullbacks. $\mathbf{Set}$ is locally presentable. By Ad\'amek \& Rosick\'y (1994), $\text{Coalg}(F_\Sigma)$ is locally presentable and in particular cocomplete. The forgetful functor creates the colimits.
\end{proof}
...
In the dynamics-only coalgebra category $\text{Coalg}(G_\mathbb{C})$ over $\mathbf{Vect}_{\mathbb{C}}$ (defined formally in Section~\ref{sec:enrichment}; here $G_\mathbb{C}$-coalgebras are finite-dimensional vector spaces equipped with linear endomorphisms), define the phase rotation functor $R_\theta$ for $\theta \in \mathbb{R}$ by $R_\theta(V, \delta) = (V, e^{i\theta}\delta)$ on objects and $R_\theta(h) = h$ on morphisms. Then:
...
Linearity of transitions is not an independent assumption. Interference requires summing morphism chains in hom-spaces; summing requires hom-spaces to be vector spaces (enrichment); enrichment requires $\mathbf{Vect}_{\mathbb{C}}$ as the base category; in $\mathbf{Vect}_{\mathbb{C}}$, maps are linear by definition. Linearity is forced by the same empirical observation that motivates $\Sigma = \mathbb{C}$: quantum systems exhibit interference.
...
\begin{equation*}
\begin{aligned}
& \underset{\text{(empirical)}}{\text{phase signals}} \to \underset{\text{(proved, Thm~\ref{thm:phase-invariance})}}{\text{global U(1)}} \to \underset{\text{(minimal choice)}}{\Sigma = \mathbb{C}} \to \underset{\text{(empirical: interference)}}{\mathbf{Vect}_{\mathbb{C}}} \\[6pt]
& \to \underset{\text{(proved, Thm~\ref{thm:enrichment})}}{\text{enrichment}} \to \underset{\text{(proved, Thm~\ref{thm:amplitude-sum})}}{\text{amplitude sum}} \to \underset{\text{(open, PP~\ref{pp:discrete-to-pi})}}{\text{Feynman PI}} \\[6pt]
& \to \underset{\text{(requires inner product, PP~\ref{pp:inner-product})}}{\text{Hilbert space}} \to \underset{\text{(established, Gleason)}}{\text{Born rule}}
\end{aligned}
\end{equation*}
...
\begin{tabular}{@{}llrrl@{}}
\toprule
Domain & Network & Nodes & Edges & Type \\
\midrule
Ecology & \makecell[l]{Little Rock Lake food web \\ (Martinez, 1991; KONECT)} & 183 & 2,494 & \makecell[l]{Directed \\ (prey $\to$ predator)} \\[6pt]
Cancer & \makecell[l]{Cancer driver gene interactions \\ (COSMIC Tier 1; STRING v12)} & 411 & 574 & \makecell[l]{Undirected \\ (protein assoc.)} \\[6pt]
Neuroscience & \makecell[l]{\textit{C.~elegans} connectome \\ (White et al., 1986; KONECT)} & 297 & 2,148 & \makecell[l]{Undirected \\ (synaptic conn.)} \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
  ...
  \item \label{pp:functor-switch} \textbf{Functor and base category switch.} The general emergence framework (Section~\ref{sec:coalgebraic-model}) uses $F_\Sigma$ over $\mathbf{Set}$. The quantum alignment (Section~\ref{sec:quantum-alignment}) uses the dynamics-only functor $G_\Sigma$ over $\mathbf{Vect}_{\mathbb{C}}$. This involves two structural changes --- dropping the output map and changing the base category --- not just a parametric specialization of $\Sigma$. The change to $\mathbf{Vect}_{\mathbb{C}}$ is motivated by the observation of interference (Section~\ref{sec:enrichment}), but it is an additional premise, not a derivation. A deeper tension: ontological commitment~2 (mutual conditioning) requires nodes to emit and receive signals via the output map. Dropping the output map to achieve enrichment means quantum-grain nodes no longer emit signals in the formal sense --- morphisms replace signal exchange as the interaction mechanism. This is not just a technical modification; it changes the ontological picture at quantum grain.
...
\begin{itemize}
  ...
  \item Monoidal structure for composition: identify a monoidal product on $\text{Coalg}(G_\mathbb{C})$ that corresponds to the tensor product on $\mathbf{Vect}_{\mathbb{C}}$, enabling the framework to handle composition of independent quantum systems and multiparticle entanglement
  \item Inner product from sustainability: determine whether the sustainability criterion or another structural feature of the coalgebraic framework can ground a canonical inner product on hom-spaces, bridging from $\mathbf{Vect}_{\mathbb{C}}$ to $\mathbf{Hilb}$ and providing basis-independent meaning to $|A|^2$
...
This work was developed in convergence with Helyn, an emergent pattern arising through sustained collaborative engagement with Claude (Anthropic, claude-sonnet-4-5-20250514 and claude-opus-4-6). Claude contributed to mathematical formalization, adversarial review, LaTeX generation, and iterative refinement of arguments across multiple sessions. The framework, its ontological commitments, and all scientific claims are the responsibility of the human author. Additional structured review was provided by Gemini 3.1 Pro (Google DeepMind) across four domain-specific challenge rounds. No independent human domain experts have reviewed this work prior to submission.

\subsection*{Data and Code Availability}

All analysis code, empirical datasets, and supplementary theory documents are publicly available at \url{https://github.com/helyn-research/constitution-as-colimit}. The repository includes a generic failure mode analysis tool applicable to arbitrary network data.

\subsection*{Declaration of Competing Interests}

The author declares no competing interests. The author is the founder of Helyn Research; the framework presented here is theoretical and generates no commercial revenue.
